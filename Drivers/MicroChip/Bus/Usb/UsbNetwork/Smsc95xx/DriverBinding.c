/** @file
  Implement the driver binding protocol for MicroChip Smsc95xx Ethernet driver.
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (EFI_ERROR(Status)) {
      ...
      DEBUG ((DEBUG_INFO, "NumConfigurations: %d, MaxPacketSize0: %d\n", Device.NumConfigurations, Device.MaxPacketSize0));
      //
...
ERR:
  ...
  if (NicDevice->BulkOutBuf != NULL) {
    gBS->FreePool (NicDevice->BulkOutBuf);
  }
...
/**
  Smsc95xx driver unload routine.
